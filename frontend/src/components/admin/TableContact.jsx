            ...
            <div className="overflow-x-auto">
                <table className="w-full bg-white min-w-max table-auto">
                    <thead className="border-b-2 border-gray-200">
                        <tr>
                            <th className="p-4 tracking-wide text-left text-xl font-semibold text-heading w-[20%]">Name</th>
                            <th className="p-4 tracking-wide text-left text-xl font-semibold text-heading w-[30%]">Email</th>
                            <th className="p-4 tracking-wide text-left text-xl font-semibold text-heading w-auto pr-8">Message</th>
                        </tr>
                    </thead>
                    <tbody>
                        {contacts.map((item) => (
                            <tr key={item.id} className="border-b-2 border-gray-200">
                                <td className="p-4 whitespace-nowrap text-xl text-gray-500">{item.name}</td>
                                <td className="p-4 whitespace-nowrap text-xl text-gray-500 hover:underline">{item.email || "-"}</td>
                                <td className="p-4 whitespace-nowrap text-xl text-gray-500 pr-8 truncate">{item.message}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
